...
test('parseCompendiumExport accepts a JSON string as well as an object', () => {
  const { build } = parseCompendiumExport(JSON.stringify(fixture));
  assert.ok(build.extraModifiers.length > 0);
  // Produces a valid master-shaped Build (empty selectable-source arrays).
  assert.deepEqual(build.memoryIds, []);
  assert.deepEqual(build.voraxGear, []);
});
