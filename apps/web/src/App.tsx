...
/** Compact "Building toward" panel: the planned build surfaced in-game. */
function GoalPanel({ goal }: { goal: Build }) {
  const summary = useMemo(() => {
    ...
    try {
      dps = evaluateBuild(goal, datasetIndex).damage.dps;
    } catch {
      dps = undefined;
    }
    return { heroName: hero?.name, skillName: skill?.name, dps };
  }, [goal]);
  ...
  return (
    <section className="hud-panel goal-panel">
      ...
      {summary.dps !== undefined && summary.dps > 0 && (
