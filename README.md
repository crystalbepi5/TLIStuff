...
- **Build interop** (`packages/build-data/src/interop.ts` + `compendium.ts`) — a tested native share-code format plus a pluggable adapter framework (`importBuildCode`). **TLI Compendium JSON exports import for real**: paste a build's exported JSON and its gear + Divinity-slate affixes are parsed into modifiers (33 from the sample "Pharma" build). What the export *doesn't* name — the main skill, hero traits, hero-memories, skill-tree nodes — is stored as opaque guids; those resolve only against Compendium's per-season **data bundle** (e.g. `SS12.5-hero-memory-en.json`), which `parseCompendiumExport` accepts as an optional dictionary (hero-memories wired; hosted on a blocked host, so supply it locally). The PoB adapter is still a stub, and a pasted editor URL (no build in it) returns a message telling you to Export the JSON instead.
