...
- **`apps/web`** — a **Build Planner** view: pick a hero, main skill, supports, gear, and **progression** (talent-tree nodes, Pact Spirits, and SS13 Memory Revival awakenings), and see DPS / crit / rate / per-element damage and defence recompute live. Reachable at the `#planner` URL hash (a normal opaque page, separate from the transparent overlay). Builds export/import as a base64 share code. It also **imports a [TLI Compendium](https://tlicompendium.com/en/build-planner) build export**: paste a build's exported JSON and its gear + Divinity-slate affixes are parsed into modifiers (`packages/build-data/src/compendium.ts`) and folded into the current build, so DPS/defence reflect that character's real rolled gear. What the export stores as opaque guids — the main skill, hero traits, hero-memories, skill-tree nodes — isn't reconstructed from the export alone; hero-memories resolve when Compendium's per-season data bundle (e.g. `SS12.5-hero-memory-en.json`) is supplied to `parseCompendiumExport`.
